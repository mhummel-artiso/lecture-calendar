...
public class KeyCloakHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return response.IsSuccessStatusCode ? HealthCheckResult.Healthy() : HealthCheckResult.Unhealthy("Authentication server is not available");
        }
        catch (Exception e)
        {
            logger.LogError(e, "Keycloak health check failed");
            return HealthCheckResult.Unhealthy("Authentication server is not available");
        }
    }
}