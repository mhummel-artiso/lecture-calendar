...
namespace Calendar.Api.Validation
{
    public static class EventValidator
    {
        ...
        {
            if (calendarEvent.Rotation == EventRotation.None)
                return;
            if (!calendarEvent.EndSeries.HasValue)
                throw new ArgumentException("If rotation is given, end series must also be given.");

            var seriesEndDate =
                new DateTimeOffset(calendarEvent.EndSeries.Value.Year,
                    calendarEvent.EndSeries.Value.Month, calendarEvent.EndSeries.Value.Day,
            ...
            switch (calendarEvent.Rotation)
            {
                case EventRotation.Daily when seriesEndDate < calendarEvent.Start.AddDays(1):
                    throw new ArgumentException("SerieEnd must for daily roatation at least one day bigger than event start");
                case EventRotation.Weekly when seriesEndDate < calendarEvent.Start.AddDays(7):
                    throw new ArgumentException("SerieEnd must for weekly rotation at least one week bigger than event start");
                case EventRotation.Monthly when seriesEndDate < calendarEvent.Start.AddMonths(1):
