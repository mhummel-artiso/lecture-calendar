...
import { PassedDialogValues, AddOrEditEventDialogContent } from './content/AddOrEditEventDialogContent';
...
export const EventDialog: FC<EventDialogComponentProps> = ({
    ...
    const [askEditSeries, setAskEditSeries] = useState<PassedDialogValues|undefined>();
    ...
    const handleAddOrEditEvent = (value: PassedDialogValues|undefined, editSeries: boolean | undefined = undefined) => {
        if(!value)
            return
    ...
    return (
        <Dialog open={isDialogOpen} onClose={handleClose}>
            ...
            {askDeleteSeries && <EditEventSeriesDialogContent
                title="Serie löschen?"
                onCanceled={handleCancelAskDialog}
                onAccepted={(_,deleteSeries)=> handleDeleteClick(deleteSeries)}>
                <Typography>
                    Wollen Sie nur das aktuelle Ereignis oder die gesamte Serie löschen?
                </Typography>
            </EditEventSeriesDialogContent>}
            {!askEditSeries && !askDeleteSeries && 
                <AddOrEditEventDialogContent isSeries={isSeries} calendarId={calendarId} isEdit={isEdit} currentValue={currentValue} onDelete={() => {
                    if(isSeries) {
                        setAskDeleteSeries(true)
                    } else {
                        handleDeleteClick(false)
                    }
                }} onAccept={(value)=>isEdit && isSeries? setAskEditSeries(value) : handleAddOrEditEvent(value)} onCancel={handleClose}/>
            }
        </Dialog>
    )
}
