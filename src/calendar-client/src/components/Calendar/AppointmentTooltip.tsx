...
import { Avatar, Chip, Grid, List, ListItem, ListItemIcon, ListItemText, Typography } from "@mui/material";
import RemoveIcon from '@mui/icons-material/Remove';

export const AppointmentTooltipContent: React.FC<AppointmentTooltip.ContentProps> = ({
                                                                                         children,
                                                                                         appointmentData,
                                                                                         ...restProps
                                                                                     }) => {
    ...
    return (
        <AppointmentTooltip.Content {...restProps} appointmentData={appointmentData}>
            <Grid sx={{mt: 1, ml: 1}} container alignItems="left" direction="column" spacing={1}>
                ...
                {event.instructors.length > 0 && (<Grid item container spacing={1}>
                    ...
                    <Grid item container direction="row" spacing={1}>
                        {event.instructors.map((i) => (
                            <Grid item key={i.id}>
                                <Chip avatar={<Avatar alt={i.name}/>} label={i.name}/>
                            </Grid>
                        ))}
                    </Grid>
                </Grid>)}
...
export const AppointmentTooltipHeader: React.FC<AppointmentTooltip.HeaderProps> = ({
                                                                                       children,
                                                                                       appointmentData,
                                                                                       ...restProps
                                                                                   }) => {
